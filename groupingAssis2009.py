...
for simulation in range(5):
    for nb_learners in [100, 500, 1000]:
        ...
        for nb_tests in [50]:
            ...
            df = pd.read_json('./Data/log_data.json')

            a = list(df.logs.values)

            df_2 = []

            for idx, user in enumerate(df.user_id):
                logs = a[idx]
                for log in logs:
                    df_2.append( [user]+list(log.values()) )

            df_2 = pd.DataFrame(df_2,columns=['user_id','item_id','correct','kc'])
            ...
            df_train.to_csv(f'./res_Assis2009/train_{nb_learners}_{nb_tests}.csv',index=False)
            df_test.to_csv(f'./res_Assis2009/test_{nb_learners}_{nb_tests}.csv',index=False)
            ...
            with open(f'./res_Assis2009/learners/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
                pickle.dump(learners, fp)

            with open(f'./res_Assis2009/tests/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
                pickle.dump(tests, fp)

            with open(f'./res_Assis2009/tests2kc/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
            ...
            with open(f'./res_Assis2009/perfs/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
                pickle.dump(perf, fp)
            ...
            for k in ks:
                for m in ms:
                    ...
                    #Our AntiClust Solution - Diameter
                    beg = time.time()
                    grouping_ours_with_apt = antiClustering(perf, learners, tests, k, m, tests2kc, learning_group='diameter', init='variance', switch = True)
                    fin = time.time() - beg
                    grouping_list.append(grouping_ours_with_apt)
                    res.append([nb_learners, nb_tests, k, m, 'OursV_S', 'yes', 'diameter', fin])
                    # ---
                    beg = time.time()
                    grouping_ours_without_apt = antiClustering(perf, learners, tests, k, m, tests2kc, learning_group='diameter', init='variance', switch = True, aptitude=False)
                    fin = time.time() - beg
                    grouping_list.append(grouping_ours_without_apt)
                    res.append([nb_learners, nb_tests, k, m, 'OursV_S', 'no', 'diameter', fin])
                    

                    #Our AntiClust 2 Solution - Diameter
                    beg = time.time()
                    grouping_ours_with_apt = antiClustering(perf, learners, tests, k, m, tests2kc, learning_group='diameter', init='random', switch = True)
                    fin = time.time() - beg
                    grouping_list.append(grouping_ours_with_apt)
                    res.append([nb_learners, nb_tests, k, m, 'OursR_S', 'yes', 'diameter', fin])
                    # ---
                    beg = time.time()
                    grouping_ours_without_apt = antiClustering(perf, learners, tests, k, m, tests2kc, learning_group='diameter', init='random', switch = True, aptitude=False)
                    fin = time.time() - beg
                    grouping_list.append(grouping_ours_without_apt)
                    res.append([nb_learners, nb_tests, k, m, 'OursR_S', 'no', 'diameter', fin])
                    """"


                    #Collect
                    with open(f'./res_Assis2009/groupings/{file_name}groupings_{nb_learners}_{nb_tests}_{k}_{m}_new_{simulation}', 'wb') as fp:
                        pickle.dump(grouping_list, fp)

                    with open(f'./res_Assis2009/groupings/{file_name}res_{nb_learners}_{nb_tests}_{k}_{m}_new_{simulation}', 'wb') as fp:
                        pickle.dump(res, fp)
