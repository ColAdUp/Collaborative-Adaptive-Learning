...
parser.add_argument("-d", "--data", default='Assis2009', type=str, help="Dataset")
args = parser.parse_args()

type_skill = args.skill
dataset = args.data

if dataset not in ['Assis2009', 'Neurips', 'AssisChallenge']:
    sys.exit('Dataset does not exist')
    ...
    for nb_learners in [100, 500, 1000]:
        ...
        for nb_tests in [50]:
            ...
            if dataset == 'Assis2009':
                ...
                df = df.rename(columns={'studentId':'user_id', 'problemId':'item_id', 'skill':'kc'})
            ...
            df_train.to_csv(f'./{folder}/train_{nb_learners}_{nb_tests}.csv',index=False)
            df_test.to_csv(f'./{folder}/test_{nb_learners}_{nb_tests}.csv',index=False)
            ...
            with open(f'./{folder}/learners/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
                pickle.dump(learners, fp)

            with open(f'./{folder}/tests/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
                pickle.dump(tests, fp)

            with open(f'./{folder}/tests2kc/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
            ...
            with open(f'./{folder}/perfs/{file_name}{nb_learners}_{nb_tests}_new_{simulation}', 'wb') as fp:
                pickle.dump(perf, fp)
            ...
            for k in ks:
                for m in ms:
                    ...
                    #Collect
                    with open(f'./{folder}/groupings/{file_name}groupings_{nb_learners}_{nb_tests}_{k}_{m}_new_{simulation}', 'wb') as fp:
                        pickle.dump(grouping_list, fp)

                    with open(f'./{folder}/groupings/{file_name}res_{nb_learners}_{nb_tests}_{k}_{m}_new_{simulation}', 'wb') as fp:
                        pickle.dump(res, fp)
