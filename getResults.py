...
for simulation in range(5):
    ...
    for nb_learners in [100, 500, 1000]:
        ...
        for nb_tests in [50]:
            ...
            df_train = pd.read_csv(f'./{folder}/train_{nb_learners}_{nb_tests}.csv')
            df_test = pd.read_csv(f'./{folder}/test_{nb_learners}_{nb_tests}.csv')

            ground_truth = df_test.groupby('user_id')['item_id'].apply(list).reset_index().set_index('user_id').T.to_dict('list')

            with open(f'./{folder}/learners/{file_name}{nb_learners}_{nb_tests}_new{simulation}', 'rb') as fp:
                learners = pickle.load(fp)

            with open(f'./{folder}/tests/{file_name}{nb_learners}_{nb_tests}_new{simulation}', 'rb') as fp:
                tests = pickle.load(fp)

            with open(f'./{folder}/tests2kc/{file_name}{nb_learners}_{nb_tests}_new{simulation}', 'rb') as fp:
                tests2kc = pickle.load(fp)

            with open(f'./{folder}/perfs/{file_name}{nb_learners}_{nb_tests}_new{simulation}', 'rb') as fp:
                perf = pickle.load(fp)
            ...
            for k in ks:
                for m in ms:
                    ...
                    with open(f'./{folder}/groupings/{file_name}groupings_{nb_learners}_{nb_tests}_{k}_{m}_new{simulation}', 'rb') as fp:
                        grouping_list = pickle.load(fp)

                    with open(f'./{folder}/groupings/{file_name}res_{nb_learners}_{nb_tests}_{k}_{m}_new{simulation}', 'rb') as fp:
                        res = pickle.load(fp)
                    ...
                    all_res.to_csv(f'./{folder}/results/{file_name}all_results_new{simulation}.csv',index=False)

                    with open(f'./{folder}/detailed/{file_name}res_{nb_learners}_{nb_tests}_{k}_{m}_{simulation}', 'wb') as fp:
